...
class NaVILATrainer(BaseVLNCETrainer):
    ...
    def _eval_checkpoint(
        ...
        if has_meta_params:
            print("⚠️ Detected meta tensors — initializing on CPU first.")
            temp = model.vision_tower.vision_tower.vision_model.embeddings.position_ids.cpu()
            model = model.to_empty(device="cpu")  # allocate in CPU memory first
            model.vision_tower.vision_tower.vision_model.embeddings.position_ids = temp
            ...
            if os.path.exists(ckpt_file):
                ...
                print(f"❗Warning: checkpoint weights not found at {ckpt_file}")

            # Move to GPU after weights loaded (optional, only if enough VRAM)
            try:
                model = model.to(device, non_blocking=True)
            except RuntimeError as e:
                print(f"⚠️ Could not move fully to GPU due to limited memory: {e}")
                print("→ Keeping model on CPU for now.")
        else:
            print(f"✅ Model already on {next(model.parameters()).device}")
        ...
        while envs.num_envs > 0 and len(stats_episodes) < num_eps:
            ...
            if len(queue_actions) > 0:
                ...
                with torch.no_grad():
                    ...
                    # #### Jonny addition: ####
                    # images_tensor = images_tensor.float()  # ensure float
                    # images_tensor = images_tensor / 255.0  # normalize if still 0-255
                    # images_tensor = torch.nn.functional.interpolate(
                    #     images_tensor, size=(378, 378), mode='bilinear', align_corners=False
                    # )
